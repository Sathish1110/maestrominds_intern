...
//           outline: none;
//           box-shadow: 0 0 15px rgb(34, 33, 52);
//         }
//         .header-controls {
//           display: flex;
//           gap: 24px;
...
interface Course {
  ...
  description: string;
  imageUrl: string;
  videoUrl: string; // Added videoUrl for video upload
  progress: number; // 0-100
}
...
export const LearningHub: React.FC = () => {
  ...
  const [showNotifications, setShowNotifications] = useState(false);
  const [showAchievements, setShowAchievements] = useState(false);
  const [activeTab, setActiveTab] = useState("courses"); // New state for active tab
  const [notifications, setNotifications] = useState(notificationsData);
  ...
  const handleCourseSubmit = () => {
    const newCourseData = { ...newCourse, id: coursesData.length + 1 };
    ...
    setActiveTab("courses"); // Switch back to courses tab after submission
  };
  ...
  return (
    <Router>
        ...
        * {
        ...
        @media (max-width: 640px) {
          ...
          .submit-button {
            padding: 12px 20px;
            font-size: 1rem;
            background: linear-gradient(90deg, #4f46e5, #4338ca); /* Gradient background */
            color: white;
            border: none;
            border-radius: 8px;
            cursor: pointer;
          ...
          .submit-button:hover {
            background: linear-gradient(90deg, #4338ca, #4f46e5); /* Reverse gradient on hover */
      ...
      `}</style>
          ...
          <div className="header-inner-container">
            ...
            <nav className="header-controls" aria-label="User  actions">
              ...
              <div className="tab-switch-buttons">
                ...
                <button
                  className={`icon-button ${
                    activeTab === "postCourse" ? "active" : ""
                  }`}
                  onClick={() => setActiveTab("postCourse")}
        ...
        <button
          className={`tab-button ${activeTab === "postCourse" ? "active" : ""}`}
          onClick={() => setActiveTab("postCourse")}
        >
          Add Course
        </button>
          ...
          {activeTab === "courses" ? (
            ...
            <section>
              <div className="add-course-panel">
                ...
                />
                <button className="submit-button" onClick={handleCourseSubmit}>
                  Submit Course
                </button>
              </div>
            </section>
        ...
        {/* Define routes for the course content page */}
        <Routes>
          <Route
            path="/course/:courseId"
            element={<CourseContent courses={coursesData} />}
          />
        </Routes>
